...
public class ClientTeamManagerImpl implements ClientTeamManager {
	...
	public void initSelfDetails(UUID selfTeamID) {
		...
		UUID candidateId = profile != null && profile.getId() != null ? profile.getId() : minecraft.getUser().getProfileId();

		selfKnownPlayer = knownPlayers.get(candidateId);

		if (selfKnownPlayer == null) {
			String playerName = profile != null ? profile.getName() : minecraft.getUser().getName();

			selfKnownPlayer = knownPlayers.values().stream()
					.filter(kcp -> kcp.name().equalsIgnoreCase(playerName))
					.findFirst()
					.orElse(null);

			if (selfKnownPlayer != null) {
				FTBTeams.LOGGER.warn("Matched local player '{}' by name after UUID mismatch (client id {}, server id {}).", playerName, candidateId, selfKnownPlayer.id());
			} else if (profile != null) {
				FTBTeams.LOGGER.warn("Local player '{}' ({}) was not present in synced team data; creating provisional entry. Server authentication may be disabled.", playerName, candidateId);

				selfKnownPlayer = new KnownClientPlayer(
						candidateId,
						playerName,
						true,
						selfTeamID,
				...
				knownPlayers.put(candidateId, selfKnownPlayer);
			} else {
				FTBTeams.LOGGER.warn("Local player id {} was not found in the known players list [{}]!", candidateId,
						String.join(",", knownPlayers.keySet().stream().map(UUID::toString).toList()));
			}
		}
	}
